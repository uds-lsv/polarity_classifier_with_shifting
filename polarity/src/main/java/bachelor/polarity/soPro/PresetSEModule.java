...
/**
 * Module used to find subjective expression and shifters with preset subjective
 * expression positions.
 * 
 * @author Maximilian
 *
 */
public class PresetSEModule implements Module {
	...
	/**
	 * Constructs a new PresetSEModule.
	 * 
	 * @param sentimentLex
	 ...
	 */
	public PresetSEModule(SalsaAPIConnective salsa, SentimentLex sentimentLex, ShifterLex shifterLex,
	...
	public Collection<Frame> findFrames(SentenceObj sentence) {
		...
		// WordObjs.
		// ArrayList<WordObj> presetSEWords = new ArrayList<>();
		int wordIndex = 0;
		...
		for (Terminal terminal : presetTree.getTerminals()) {
			...
			for (String fenodeId : fenodeIds) {
				if (terminalId.equals(fenodeId)) {
					...
					// presetSEWords.add(sentence.getWordList().get(wordIndex -1));
		...
		for (WordObj sentiment : sentimentList) {
			if (sentimentLex.getAllSentiments(sentiment.getLemma()) == null) {
				...
				SentimentUnit newUnit = new SentimentUnit(sentiment.getLemma(), "UNKNOWN", "0.0", sentiment.getPos(), Boolean.FALSE);
		...
		for (WordObj word : sentence.getWordList()) {
			...
			/*
			 * ArrayList<SentimentUnit> sentLexEntries =
			 * sentimentLex.getAllSentiments(word.getLemma()); if (sentLexEntries !=
			 * null) { for (SentimentUnit sentLexEntry : sentLexEntries) { if
			 * (sentLexEntry != null) { if (posLookupSentiment) {
			 * posLookupSentiment(sentimentList, word, sentLexEntry); } else {
			 * sentimentList.add(word); break; } } } }
			 */
		}
